...
async function sendDynamicEmail(subject, detailsBlock) {
  ...
  try {
    ...
    // EmailJS throws an object that typically contains { status, text }
    const errorDetail = err.text || err.message || JSON.stringify(err);
    throw new Error(`EmailJS Error: ${errorDetail}`);
  }
}
...
export function getWhatsAppUrl(serviceType = '') {
  ...
  const text = encodeURIComponent(
    `Hi GapAnchor team, I WOULD LIKE TO ENQUIRE ABOUT ${serviceType ? serviceType.toUpperCase() : 'YOUR SERVICE'}`
  );
  return `https://wa.me/${number}?text=${text}`;
}

export function sendPartnerInquiry(formData) {
  const details = Object.entries(formData)
    .filter(([key]) => key !== 'subscribe') // optional: filter out internal fields
    .map(([key, value]) => {
      // Convert camelCase to Title Case for better readability in email
      const label = key.replace(/([A-Z])/g, ' $1').replace(/^./, str => str.toUpperCase());
      return `${label}: ${Array.isArray(value) ? value.join(', ') : value || 'N/A'}`;
    })
    .join('\n');
    
  return sendDynamicEmail('NEW WMS/SUPPLY CHAIN PARTNER INQUIRY', details);
}
